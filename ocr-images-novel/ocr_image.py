...
def main():
    if len(sys.argv) < 2:
        print("用法: python ocr_image.py <图片路径> [参数]")
        print("\n参数:")
        print("  --output FILE      输出文件路径")
        print("  --chunk-height N   分块高度 (默认: 2000)")
        print("  --overlap N        重叠像素 (默认: 200)")
        print("  --no-enhance       不进行图片增强")
        print("  --paddle           使用 PaddleOCR (需额外安装)")
        print("\n示例:")
        print("  python ocr_image.py image.png")
        print("  python ocr_image.py image.png --output result.txt")
        print("  python ocr_image.py image.png --chunk-height 3000 --overlap 300")
        print("  python ocr_image.py image.png --paddle")
        sys.exit(1)
